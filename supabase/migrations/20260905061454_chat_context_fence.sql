...
alter table public.chat_messages add constraint chat_context_position_key unique (account_id,context_generation,thread,position);
create unique index chat_context_external_key on chat_messages (account_id,context_generation,channel,external_scope,external_msg_id) where external_msg_id is not null;
create index chat_context_history_idx on public.chat_messages (account_id,context_generation,thread,created_at desc,position desc);

create function unc_private.guard_chat_context_generation() returns trigger
language plpgsql security invoker set search_path='' as $$
declare generation bigint;
begin
  if tg_op='UPDATE' and (new.id is distinct from old.id or new.account_id is distinct from old.account_id
    or new.context_generation is distinct from old.context_generation or new.thread is distinct from old.thread
    or new.position is distinct from old.position or new.channel is distinct from old.channel
    or new.external_scope is distinct from old.external_scope or new.external_msg_id is distinct from old.external_msg_id
    or new.created_at is distinct from old.created_at) then
    raise exception 'Chat identity cannot be reassigned' using errcode='23514';
  end if;
  ...
  if not found or new.context_generation is distinct from generation then
    raise exception 'Chat business context changed' using errcode='40001';
  end if;
  -- App conversation and current opt-out audit remain available during a setup hold.
  -- This guard does not authorize channel sends or replace their binding/outbox controls.
  return new;
end;
$$;
revoke all on function unc_private.guard_chat_context_generation() from public,anon,authenticated;
create trigger chat_context_generation before insert or update on public.chat_messages
  for each row execute function unc_private.guard_chat_context_generation();

-- The existing autosave revision is checked under the same account lock that captures
-- generation. Never stamp an unchecked stale payload into a reset context.
create or replace function public.save_account_state_atomic(
  p_account_id uuid, p_user_id uuid, p_expected_revision bigint, p_save_id uuid, p_rows jsonb
) returns jsonb
language plpgsql security invoker set search_path = '' as $$
declare
  v_generation bigint; v_revision bigint; v_last_id uuid; v_last_hash text; v_hash text;
...
begin
  ...
  select context_generation into v_generation from public.accounts where id=p_account_id for update;
  ...
  for v_item in select value from jsonb_array_elements(p_rows->'chatMessages') loop
    ...
    if coalesce(v_m.context_generation,0)<>v_generation then
      raise exception 'Autosave chat business context changed' using errcode='40001';
    end if;
    ...
    if exists(select 1 from public.chat_messages where account_id=p_account_id and context_generation=v_generation and thread=v_m.thread and position=v_m.position and channel<>'app') then
      raise exception 'channel collision' using errcode='22023';
    end if;
    insert into public.chat_messages(account_id,context_generation,thread,position,lane,sender,body,meta,channel)
      values(p_account_id,v_generation,v_m.thread,v_m.position,v_m.lane,v_m.sender,v_m.body,v_m.meta,'app')
      on conflict(account_id,context_generation,thread,position) do update set lane=excluded.lane,sender=excluded.sender,body=excluded.body,meta=excluded.meta;
  end loop;
...
language sql stable security invoker set search_path = '' as $$
  select jsonb_build_object(
    ...
    'chatMessages', coalesce((select jsonb_agg(to_jsonb(m) order by m.position) from public.chat_messages m where m.account_id=a.id and m.channel='app' and m.context_generation=a.context_generation),'[]'),
