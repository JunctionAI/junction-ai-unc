...
-- Historical/legacy runs remain generation zero; never label them with today's
-- account generation. New code supplies the value captured before reading inputs.
alter table public.routine_runs add column context_generation bigint not null default 0 check (context_generation >= 0);

-- Trigger-only, not an API RPC. Definer privilege is confined to locking/reading
-- account controls and parent runs; original table grants/RLS still authorize DML.
create function unc_private.guard_runtime_run_context() returns trigger
...
begin
  ...
  if tg_op='UPDATE' then
    ...
    elsif new.run_id is distinct from old.run_id then
  ...
  if tg_table_name='routine_runs' then
    ...
    if new.run_id is null then
      if generation<>0 then raise exception 'A captured parent run is required for this context' using errcode = '40001'; end if;
      return new;
    end if;
...
$$;
revoke all on function unc_private.guard_runtime_run_context() from public,anon,authenticated,service_role;

create trigger runtime_run_context before insert or update on public.routine_runs for each row execute function unc_private.guard_runtime_run_context();
create trigger runtime_run_context before insert or update on public.approvals for each row execute function unc_private.guard_runtime_run_context();
create trigger runtime_run_context before insert or update on public.artifacts for each row execute function unc_private.guard_runtime_run_context();
create trigger runtime_run_context before insert or update on public.receipts for each row execute function unc_private.guard_runtime_run_context();

-- The table already has client SELECT only; explicitly preserve the new column's
-- server-only write boundary. A failed client mutation cannot rebase an old run.
revoke insert(context_generation), update(context_generation) on public.routine_runs from public,anon,authenticated;
commit;
