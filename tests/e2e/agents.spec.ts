...
test("single and combined selections persist across reload without any run request",async({page})=>{
  ...
  await expect(page.getByText("2 of 35 catalog routines selected")).toBeVisible();await page.reload();await expect(first).toBeChecked();
  await first.click();await expect(first).not.toBeChecked();await expect(first).toBeEnabled();
  await page.getByRole("switch",{name:"Plan next month’s campaigns",exact:true}).click();await expect(page.getByText("0 of 35 catalog routines selected")).toBeVisible();
...
for(const width of [390,1280])test(`Agents fits ${width}px`,async({page})=>{
  ...
  await expect(page.getByText("0 of 35 catalog routines selected")).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);expect(errors).toEqual([]);
