...
export function useManualRecovery(context:AgentContext|undefined,routineId:string,purpose:string) {
  ...
  const [outcome,setOutcome]=useState<{key:string|null;phase:string|null;status:string|null}|null>(null);
  const phase=outcome?.key===key?outcome?.phase??null:null,status=outcome?.key===key?outcome?.status??null:null;
  ...
  async function submit(path:string,body:Record<string,unknown>) {
    ...
    setOutcome({key,phase:String(data.phase),status:String((data.run as {status:string}).status)});return data;
  }
  ...
  const terminal=phase==="claimed" && (["done","failed","skipped"].includes(status??"") || purpose==="input" && status==="waiting_input");
  function clear() {if(!key || !terminal)return;clearManualJournal(key);setOutcome(null);}
  return {journal,phase,status,error,terminal,submit,recover,clear,blocked:!!journal || !!error};
}
...
export default function ManualRecovery({recovery,busy,blocked,onResult}: {recovery:ReturnType<typeof useManualRecovery>;busy:boolean;blocked?:boolean;onResult:(body:Record<string,unknown>)=>void}) {
  ...
  async function check(continueOriginal:boolean) {
    if(inFlight.current || busy)return;inFlight.current=true;setChecking(true);setError(null);
    try{onResult(await recovery.recover(continueOriginal));}catch(e){setError(e instanceof Error?e.message:String(e));}
  ...
  return <div role="status" style={{fontSize:12,marginTop:10,lineHeight:1.6}}>
    {recovery.journal && <>Request {recovery.journal.requestId.slice(0,8)} · {recovery.phase==="prepared"?"prepared, not started":recovery.status??"outcome needs checking"}.{" "}
      <button disabled={busy||checking} onClick={()=>void check(false)}>Check original run</button>{" "}
      {recovery.phase==="prepared" && <button disabled={busy||checking||blocked} onClick={()=>void check(true)}>Continue original request</button>}
