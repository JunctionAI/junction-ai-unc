...
type RunResponse = {
  run?: { runId: string; status: string; summary: string; error?: string | null; receipts: { id: string; kind: string; description: string }[] };
  error?: string;
};
      ...
      {run && (
        <div style={{ marginTop: 12 }}>
          <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink)" }}>
            {run.status === "done" ? "Dry run complete" : run.status === "skipped" ? "Nothing to do today" : run.status === "failed" ? "Run failed closed" : run.status} <span style={{ color: "var(--muted)", fontWeight: 500 }}>— {run.summary}</span>
          </div>
