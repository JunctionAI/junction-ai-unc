...
async function handle(req: Request, save: boolean) {
  try {
    ...
    if (result.error) return json({ error: "Keyword setup is unavailable for this account or its settings changed." }, result.error.code === "42501" ? 403 : result.error.code === "40001" ? 409 : 503);
    ...
    if (saved.error) return json({ error: "Settings changed or work is outstanding. Turn the routine off, resolve any draft or pending request, then refresh." },
      saved.error.code === "42501" ? 403 : saved.error.code === "40001" ? 409 : 503);
