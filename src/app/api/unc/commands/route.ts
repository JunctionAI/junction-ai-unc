...
import { accountContextGeneration, contextRequestMatches, contextStillCurrent, contextChangedResponse } from "@/lib/db/contextGeneration";
...
async function handleGET(req: Request) {
  ...
  if (!id) {
    const commands = await unwrap("commands.own", session.service.from("routine_commands").select("id, routine_id, version, status, reply, run_id, created_at").eq("account_id", session.accountId).eq("context_generation", generation).eq("user_id", session.userId).order("created_at", { ascending: false }).limit(30));
    if (!await contextStillCurrent(session.service, session.accountId, generation)) return contextChangedResponse();
  ...
  if (!await contextStillCurrent(session.service, session.accountId, generation)) return contextChangedResponse();
  if (!c || c.actor.userId !== session.userId || c.contextGeneration !== generation) return Response.json({ error: "request not found" }, { status: 404 });
