...
/** Calendar and paid lanes carry their own accepted result-builder pin. Match the received
 * business envelope against that execution's saved output, not merely its request/ID. */
export function projectResultShadowExecution(value: unknown, binding: ExecutionEvidenceBinding & { resultNodeId: string }): ShadowExecutionObservation & { resultDigest: string } {
  ...
  if (!resultNode || resultNode.disabled === true || !text(resultNode.name, 200) ||
    ...
    return fail("result_binding");
  const runs = object(object(object(row.data)?.resultData)?.runData)?.[resultNode.name];
  if (!Array.isArray(runs) || runs.length !== 1) return fail("result_runs");
  ...
  if (!task || task.error != null || task.executionStatus !== "success" ||
    ...
    return fail("result_items");
  const body = object(object(main[0][0])?.json);
  if (!object(body?.artifact) || !object(body?.executionReceipt)) return fail("result_envelope");
  return { ...observed, resultDigest: shadowRequestDigest({ artifact: body!.artifact, executionReceipt: body!.executionReceipt }) };
}
export const projectCalendarShadowExecution = projectResultShadowExecution;
