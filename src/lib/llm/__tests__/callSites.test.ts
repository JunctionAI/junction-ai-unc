...
vi.mock("@/lib/llm/accountContext", () => ({ requireModelAccountContext: async () => {
  const db = new FakeSupabase();
  db.insertRow("accounts", { id: "acct-1", context_generation: 0, automation_paused: false });
  return { accountId: "acct-1", db, contextGeneration: 0 };
} }));
