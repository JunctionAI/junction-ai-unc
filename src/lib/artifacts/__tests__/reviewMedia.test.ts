...
describe('private review media (simulated storage and session)',()=>{
 ...
 it('refuses a future or mismatched output revision',async()=>{const d=setup();expect((await reviewMedia(request(`account=${id}&generation=1&revision=2`),`${id}_image`,d)).status).toBe(404);expect(d.download).not.toHaveBeenCalled();});
 it('serves a verified historical version without changing current version',async()=>{const d=setup();d.data.output.revision=2;const r=await reviewMedia(request(),`${id}_image`,d);expect(r.status).toBe(200);expect(d.rpc).toHaveBeenCalledWith('read_review_version',{acct:id,generation:1,actor:other,output:id,selected_revision:0});expect(d.download).toHaveBeenCalledWith(`${id}/1/${id}/0/image.png`,5);});
