...
async function seed() {
  ...
  const c: RoutineCommand = { id: commandId(actor), actor, requestHash: digest("run"), request: "run", routineId: "D01-W01", version: 1, specHash: "s", workflowHash: "w", status: "done", reply: "Draft ready", runId: "run-a", createdAt: "2026-09-04T01:00:00Z", updatedAt: "2026-09-04T01:00:00Z" };
...
describe("app dispatch boundary", () => {
  it("derives identity from the session and never from client context", async () => {
    const { db } = await seed();
    modelAccount.current = { accountId: "account-a", userId: "owner-a", db };
    ...
    expect(router.route.mock.calls[0][2]).toEqual({ accountId: "account-a", userId: "owner-a", channel: "app", requestId: "message-a" });
