...
export async function loadAccountFacts(client: DbClient, accountId: string): Promise<AccountFacts> {
  ...
  type ConnRow = { platform: string; status: ConnectorStatus; last_sync_at: string | null; last_sync_result: string | null; external_ref: string | null };
  type RsRow = { routine_id: string; enabled: boolean };
  type PlanRow = { title: string; phases: PlanPhaseJson[]; agreed_at: string | null } | null;
  type RpRow = { budget_monthly: number | string | null; hours_weekly: number | string | null; skills: string[] | null; postures: string[] | null } | null;
  const resourceNumber = (v: number | string | null): number | null => v === null || v === "" || !Number.isFinite(Number(v)) ? null : Number(v);
  ...
  const [conns, rs, plan, rp, aps, runs, rcs] = await Promise.all([
    unwrap<ConnRow[]>("connectors.select", by("connectors", "platform, status, last_sync_at, last_sync_result, external_ref")),
  ...
  return {
    accountId,
    connectors: (conns ?? []).map((c) => ({ platform: c.platform, name: NAME_BY_PLATFORM[c.platform] ?? c.platform, status: c.status, lastSyncAt: c.last_sync_at, lastSyncResult: c.last_sync_result, externalRef: c.external_ref ?? null })),
    ...
    resources: rp ? { budgetMonthly: resourceNumber(rp.budget_monthly), hoursWeekly: resourceNumber(rp.hours_weekly), skills: rp.skills ?? [], postures: rp.postures ?? [] } : null,
