...
*Generated 2026-09-02T03:35:34.479Z by `npx playwright test tests/e2e/parity.spec.ts` (scripts/parity/capture.ts).*
...
**Pass threshold.** ≤ 3% on layout-equal screens. Result: **7 of 7 screens pass.**
...
- **Typographic vs straight apostrophes** — the prototypes use straight `'` in the hero (`He'll`), the closer (`He's`) and the step-0 welcome (`Let's`); the port renders `’` (`&rsquo;` / `&apos;` in `src/app/page.tsx` and `src/components/platform/Onboarding.tsx`). Glyph-level only; the harness normalises quotes when asserting copy. Pick one and make prototype and port agree.
