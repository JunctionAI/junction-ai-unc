...
2. **No customer-facing send, publish, spend, price change, or destination mutation occurs without exact run-level authority.** `LIVE_MODE_ENABLED = false` in `src/worker/service.ts`; per-risk enablement via `UNC_LIVE_ACTION_RISKS`; the engine structurally refuses execute without an approved, unexpired gate on the same run; spend caps fail closed; reads, drafts, decisions, and attempted mutations write **receipts**. Owner-linked morning briefs, draft notices, and approval reminders are a separate founder-notification lane and can deliver automatically once that channel is explicitly linked.
...
- GitHub: verify `gh auth status` immediately before using CLI auth. A prior login is not a current receipt; if it is expired, push may still work through Git's credential helper but PR creation needs fresh authorized GitHub access.
