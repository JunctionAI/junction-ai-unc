...
- `scripts/verify-dataset-sync-fence.mjs /tmp/unc-manual-pg.x8Y6jR`: **28 real
...
- Tests include actual anon/member denial, valid server acceptance, preserved
  ...
  connector / lease lock waits crossing expiry, and successor survival/completion.
...
Worker release/readback follows this source commit. The app does not invoke
...
Next: verify the running worker's exact code and refusal path, then finish source
coverage/cadence and the remaining routine adapters. No scheduled-refresh or
