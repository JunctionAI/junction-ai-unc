...
| Variable | Where | What |
|---|---|---|
| `N8N_SIGNING_SECRET` | app + worker + n8n | **required** — HMAC for both directions and the data token. The bridge refuses to call unsigned; the proxy answers 503 without it. `openssl rand -hex 32`. |
...
| `N8N_ALLOW_HTTP` | app | `1` to allow plain-http webhook URLs (dev n8n). `localhost` is always allowed. |
...
```json
{
  ...
  "data": { "scopes": ["gorgias:tickets", "linkedin:posts", "shopify:products", "gorgias:*", "linkedin:*", "shopify:*"], "expiresAt": "2026-09-03T07:15:00.000Z",
...
`dataToken` = `unc_dt.<base64url claims>.<base64url HMAC-SHA256(N8N_SIGNING_SECRET, claims)>` with
claims `{ v: 1, accountId, runId, routineId, scopes, iat, exp }`:

- **15-minute expiry**, bound to **one run**; the proxy answers only while that run is `running`
  (`409` once it finished) — replay-safe by runId. A test call from the Skills page carries a
  `test:…` run id, which the proxy honours until the token expires (no run, no receipts).
- **Scopes** come from the routine's spec: every read node's `platform:resource`, plus
  `platform:*` for the skill minimum's required and helpful platforms. A founder-content token
  cannot read Meta insights (`403`).
...
```json
{ "platform": "meta_ads", "action": "update_adset_budget", "params": { "adsetId": "…", "increase": 20 }, "target": {…}, "title": "…", "why": "ROAS 3.1 over 7d" }
...
n8n workflow: update_adset_budget on meta_ads" under needs-you and can approve or hold — the
...
arrives with wave 2, behind the same gate, spend caps and receipts as every other mutation.
...
| `202` | (empty) | accepted: the run stays `running` with a snapshot; the workflow POSTs the callback later (the data token stays valid until it expires) |
...
Webhook trigger → HMAC verify → `/api/n8n/context` + `/api/n8n/reads` with the data token → your
automation → `{ artifact }` (or `{ needs }`). Import into n8n (Workflows → Import from file); set
`N8N_SIGNING_SECRET` in n8n's environment. Two worked examples follow; they differ only in the
reads they ask for and the artifact they shape.
...
```json
{
  ...
  "connections": {
    ...
    "Verify signature": { "main": [[{ "node": "Context", "type": "main", "index": 0 }, { "node": "Read products", "type": "main", "index": 0 }]] },
...
```json
{
  ...
  "nodes": [
    ...
    { "parameters": { "url": "={{ $json.dataBaseUrl }}/api/n8n/context", "sendHeaders": true, "headerParameters": { "parameters": [{ "name": "Authorization", "value": "=Bearer {{ $json.dataToken }}" }] }, "options": { "timeout": 20000 } }, "id": "3", "name": "Context", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4.2, "position": [440, -120] },
    { "parameters": { "url": "={{ $('Verify signature').item.json.dataBaseUrl }}/api/n8n/reads", "sendQuery": true, "queryParameters": { "parameters": [{ "name": "platform", "value": "shopify" }, { "name": "resource", "value": "products" }, { "name": "limit", "value": "50" }, { "name": "fields", "value": "[\"title\",\"body_html\",\"tags\",\"product_type\"]" }] }, "sendHeaders": true, "headerParameters": { "parameters": [{ "name": "Authorization", "value": "=Bearer {{ $('Verify signature').item.json.dataToken }}" }] }, "options": { "timeout": 20000 } }, "id": "4", "name": "Read products", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4.2, "position": [440, 120] },
...
"Immediately", response code 202), do the work — the data token is good for 15 minutes — then an
HTTP Request node POSTs the callback with the two headers computed the same way
(`x-unc-timestamp` = `Date.now()`, signature over `${ts}.${body}`).
